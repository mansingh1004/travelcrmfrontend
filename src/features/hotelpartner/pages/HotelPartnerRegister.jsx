...
function toForm(dto) {
  return {
    ...
    cityName: dto?.cityName ?? "",
    cityCode: dto?.cityCode ?? "",
    /* street and pincode have no column on the registration DTO yet. They round-trip through the
       form and go out on the payload like every other key (toPayload spreads the whole object), so
       they persist the day the backend adds them and cost nothing until then — the server ignores
       what it does not map. Until it does, a reload comes back with "" for both. */
    street: dto?.street ?? "",
    address: dto?.address ?? "",
    pincode: dto?.pincode ?? "",
    latitude: dto?.latitude ?? "",
          ...
          <Card id="location" title="Location" hint="The city and country decide where travel agents find you.">
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <Field label="PIN Code">
                <input className={inputCls} value={form.pincode} disabled={ro} maxLength={10}
                  inputMode="numeric" autoComplete="postal-code"
                  onChange={(e) => patch({ pincode: e.target.value })} placeholder="403516" />
              </Field>
              <Field label="City / airport code" hint="optional">
                <input className={inputCls} value={form.cityCode} disabled={ro} maxLength={20}
                  autoCapitalize="characters"
