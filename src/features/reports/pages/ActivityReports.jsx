...
//     let alive = true;
//     setLoading(true);
//     // Fetch all logs in the date range; action/userType/user filtering + pagination stay client-side.
...
//       page:      1,
//     })
//       .then((res)=>{ if(alive) setLogs(res.data?.logs || []); })
//       .catch(()=>{ if(alive){ setLogs([]); showToast("Failed to load activity logs.", "error"); } })
...
//     setApplied({startDate:DEFAULT_START,endDate:DEFAULT_END,action:"All Actions",userType:"All Types",user:"All Users"});
//     setPage(1);
...
//   /* user dropdown options — derived from the loaded logs (real data, not hardcoded) */
//   const userOptions = useMemo(
//     () => ["All Users", ...Array.from(new Set(logs.map(l=>l.user).filter(Boolean)))],
//     [logs]
//   );
...
//                   <FilterSelect value={user} onChange={setUser} options={userOptions}/>
//                 </div>
//                 <div>
...
//                 const tc = TYPE_CFG[log.type]     || { bg:"bg-slate-400", text:"text-white" };
//                 return (
//                   <div key={log.publicId} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3 fade-up"
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md z-10 max-h-[90vh] overflow-y-auto"
        style={{animation:"popIn .25s ease both"}}>
        {/* Modal header */}
        <div className="bg-gradient-to-r from-blue-600 to-blue-500 px-5 sm:px-6 py-5 rounded-t-2xl flex items-center justify-between sticky top-0 z-10">
          <div className="flex items-center gap-3 min-w-0">
            <div className="w-9 h-9 rounded-xl bg-white/20 flex items-center justify-center text-white flex-shrink-0">
              <FiEye className="w-4 h-4"/>
            </div>
            <div className="min-w-0">
              <h2 className="text-white font-extrabold text-base truncate">Activity Details</h2>
              <p className="text-white/70 text-xs mt-0.5">{log.date} {log.time}</p>
            </div>
          </div>
          <button onClick={onClose}
            className="w-8 h-8 rounded-full bg-white/20 hover:bg-white/30 text-white flex items-center justify-center text-lg transition-all flex-shrink-0">×</button>
        </div>
        {/* Modal body */}
        <div className="p-5 sm:p-6 space-y-3">
          {[
            ...
            <div key={label} className="flex flex-col sm:flex-row sm:items-start gap-1 sm:gap-3 py-2.5 border-b border-slate-100 last:border-0">
              <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wide sm:w-28 flex-shrink-0 pt-0.5">{label}</span>
              <span className={`text-sm font-semibold text-slate-700 break-words ${mono?"font-mono":""}`}>{value}</span>
            </div>
          ))}
          <div className="flex items-center gap-3 py-2.5 border-b border-slate-100">
            <span className="text-xs font-extrabold text-slate-500 uppercase tracking-wide w-28 flex-shrink-0">Type</span>
...
/* ─── PAGINATION ─────────────────────────────────────────────── */
/* Kept as its own component: on narrow screens (< sm) it renders a
   compact Prev / "Page x of y" / Next control so nothing overflows
   or gets clipped off the edge of the viewport. From sm upward it
   renders the full first/prev/numbers/next/last control. */
function Pagination({ page, totalPages, setPage }) {
  ...
  return (
    <div className="flex items-center gap-1.5">
      {/* Mobile: compact prev / page indicator / next — always fits, never clipped */}
      <div className="flex sm:hidden items-center gap-2">
        <button disabled={page===1} onClick={()=>setPage(p=>p-1)}
          className="w-9 h-9 rounded-lg bg-white border border-slate-200 text-slate-500 hover:border-blue-300 hover:text-blue-600 transition-all disabled:opacity-40 disabled:cursor-not-allowed flex items-center justify-center">
          <FiChevronLeft className="w-4 h-4"/>
        </button>
        <span className="text-xs font-bold text-slate-600 px-1 whitespace-nowrap">Page {page} of {totalPages}</span>
      ...
      {/* sm and up: full pagination with jump-to-first/last */}
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50/30 to-slate-100"
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <button onClick={()=>navigate("/ReportsDashboard")}
              className="flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl border border-slate-200
                hover:border-blue-300 bg-white hover:bg-blue-50 text-slate-600 hover:text-blue-600
                text-sm font-bold transition-all shadow-sm w-full sm:w-auto">
              <FiArrowLeft className="w-4 h-4"/> Back to Reports
            </button>
          </div>
        </div>
      ...
      <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 space-y-5">
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden fade-up">
          ...
          <button
            type="button"
            onClick={() => setActivityStatsOpen((previous) => !previous)}
            aria-expanded={activityStatsOpen}
            className="w-full px-4 sm:px-5 py-3 flex items-center justify-between gap-3 hover:bg-slate-50 transition-colors"
          >
            <div className="flex items-center gap-3 flex-wrap min-w-0">
            ...
            <div className="w-8 h-8 rounded-lg bg-slate-100 text-slate-500 flex items-center justify-center flex-shrink-0">
              {activityStatsOpen ? <FiChevronUp className="w-4 h-4" /> : <FiChevronDown className="w-4 h-4" />}
            </div>
          </button>
          ...
          <div
            className={`grid transition-all duration-300 ease-in-out ${
              activityStatsOpen ? "grid-rows-[1fr] opacity-100" : "grid-rows-[0fr] opacity-0"
            }`}
          >
            <div className="min-h-0 overflow-hidden">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 p-4 pt-1">
          ...
          {filtersOpen && (
            <div className="p-4 sm:p-5 space-y-4">
              ...
              <div className="flex flex-col sm:flex-row flex-wrap gap-3 pt-1">
                ...
                <button onClick={handleReset}
                  className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl border-2 border-slate-200 hover:border-slate-300
                    bg-white hover:bg-slate-50 text-slate-600 hover:text-slate-800 text-sm font-bold transition-all">
                  <FiRefreshCw className="w-3.5 h-3.5"/> Reset
                </button>
                <button onClick={handleExportCSV}
                  className="flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-bold
                    shadow-md shadow-emerald-200 transition-all">
                  <FiDownload className="w-3.5 h-3.5"/> Export CSV
                </button>
              </div>
        ...
        <div className="bg-white/80 backdrop-blur-md rounded-2xl border border-slate-200/60 shadow-sm overflow-hidden fade-up">
                ...
                return (
                  <div key={log.publicId} className="bg-white rounded-2xl border border-slate-100 shadow-sm p-4 space-y-3 fade-up"
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="min-w-0">
                        <p className="text-sm font-extrabold text-slate-800 truncate">{log.user}</p>
                        <p className="text-xs text-slate-400 truncate">{log.username}</p>
                      </div>
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed break-words">{log.description}</p>
                    <div className="flex items-center justify-between gap-2 pt-1 border-t border-slate-100">
                      <div className="min-w-0">
                        <p className="text-xs font-bold text-slate-600 truncate">{log.date} <span className="text-slate-400">{log.time}</span></p>
                        <p className="text-xs text-slate-400 font-mono truncate">{log.ip}</p>
                      </div>
                      <button onClick={()=>setDetail(log)}
                        className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-50 border border-blue-200 text-blue-600 text-xs font-bold transition-all hover:bg-blue-100 flex-shrink-0">
                        <FiEye className="w-3 h-3"/> Details
                      </button>
                    </div>
                  </div>
          ...
          {/* ── PAGINATION ── */}
          {filtered.length > 0 && (
            <div className="px-4 sm:px-5 py-4 border-t border-slate-100 bg-slate-50/60 flex flex-col sm:flex-row items-center justify-between gap-3">
