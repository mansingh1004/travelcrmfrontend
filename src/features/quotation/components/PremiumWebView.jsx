...
// /* ════════════════════════════════════════════════════════════════
//    PREMIUM WEB VIEW — v3 "Grand Brochure"
//    Rebuilt to match the TravelTales reference (navy · gold · ivory,
//    dense sectioned brochure) replacing the earlier quiet-letterhead
//    design. Update the Premium card art/description in
//    QuotationStyleModal to match.

//    Layout order mirrors the reference:
//    hero header (photo + quotation info panel) → prepared-for /
//    trip-overview / prepared-by strip → grand-total band (gold
//    figure + badge + per-adult + photo) → itinerary highlights
//    timeline with photo strip → flight table (when present) →
//    inclusions summary icons → inclusions/exclusions → hotel stay
//    table → sightseeing photo grid → vehicle cards → cruise ledger →
//    price summary / payment / add-ons / passengers grid → policies →
//    navy footer with script sign-off + QR.

//    Contract unchanged:
//    • Receives the already-fetched public DTO as `data` (NEVER fetches).
//    • `pdfUrl` optional. Section gating via q.<x>.included flags.
//    • Tolerant resolvers for images and optional fields — anything
//      the DTO doesn't carry (validUntil, bookingAmount,
//      paymentSchedule, importantNotes, customer email/phone) simply
//      doesn't render.
//    • NO agency markup anywhere — customer-facing page.
//    Fonts: Cormorant Garamond (serif) · Jost (sans) · Great Vibes (script).
//    ════════════════════════════════════════════════════════════════ */
...
//         .pw-vgrid { display:grid; grid-template-columns:repeat(auto-fill,minmax(190px,1fr)); gap:12px; }
//         .pw-vcard { border:1px solid var(--hair); border-radius:10px; overflow:hidden; background:#fdfaf1; }
//         .pw-vcard img { width:100%; height:120px; object-fit:cover; }
//         .pw-vc-b { padding:10px 12px; text-align:center; }
...
//                         {img && <ZoomImg src={cdn(img, CARD)} alt={v.model || v.type} onZoom={z(img, v.model || v.type)} />}
